...
public class VehicleAudio : MonoBehaviour
	...
	void DoEngineAudio (){
		...
		if (m_gear != m_lastGear)
			...
			//m_engineRpm = Mathf.Lerp(old_rpm, m_vehicle.engineRPM, m_engineRpmDamp * Time.deltaTime);
			//m_engineRpm = GetComponent<AdvancedVehicleController>().engine.rpm;
            ...
            //m_engineThrottleRpm = m_engineRpm;
    ...
    void DoEngineExtraAudio ()
		...
		if (engineExtras.turboDumpClip != null)
            ...
            //float transmissionRatio = Mathf.Abs(m_vehicle.engineRPM) / (engine.maxRpm * engine.gears * engineExtras.transmissionMaxRatio);
	...
	void PlayOneTime (AudioClip clip, Vector3 position, float volume, float pitch)
		...
		if (audioClipTemplate != null)
			{
			go = (UnityEngine.GameObject)Instantiate(audioClipTemplate.gameObject, position,Quaternion.identity);
			source = go.GetComponent<AudioSource>();
			go.transform.parent = audioClipTemplate.transform.parent;
			}
		else
			{
			go = new UnityEngine.GameObject("One shot audio");
