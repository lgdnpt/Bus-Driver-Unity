...
public class VehicleController : MonoBehaviour {
    ...
    void FixedUpdate(){
		...
		// Ensure input values within range确认输入值在范围内
	...
	float FixSteerAngle (WheelData wd, float inputSteerAngle) {
 	...
 	void UpdateSuspension (WheelData wd) {
		...
		if (wd.grounded && !disableWheelHitCorrection) {
            ...
            bool collided = Physics.Raycast(wd.origin, -wd.transform.up, out rayHit, wd.collider.suspensionDistance + wd.collider.radius, Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
            if (collided) {
	    ...
	    void ComputeTireForces(WheelData wd) {
		    // 本轮胎的驱动力 Throttle for this wheel
            ...
            if (brakeInput<0.3f && handbrakeInput<0.3f) {
                combinedInput = -rollingResistance + absThrottleInput * (1.0f + rollingResistance) - wheelBrakeInput * (1.0f - rollingResistance);
            } else {
                combinedInput = -rollingResistance - wheelBrakeInput * (1.0f - rollingResistance);
            }
            ...
            // 计算来自车轮车轴的受力 Calculate demanded force coming from the wheel's axle
            float demandedForce;
		    ...
		    if (wd.grounded) {
                ...
                // 计算可用的轮胎总力 Calculate the total tire force available
                ...
                // 确保有足够的纵向滑移所需的纵向力 Ensure there's longitudinal slip enough for the demanded longitudinal force
                float minSlipY;
                ...
                // 计算组合轮胎力 Compute combined tire forces
                ...
                // 表面阻力 Drag force as for the surface resistance
            ...
            // 为下一步计算角速度 Compute angular velocity for the next step
    ...
    // 计算轮胎数据的方法  Methods for calculating tire data


    // 侧向力应用点 Application point for the sideways force
    ...
    // 基于速度的滑移角 Slip angle based on velocity
    ...
    // 根据速度计算当前最大驱动力 Calculate current maximum force according to speed
	float ComputeDriveForce (float demandedForce, float maxForce, bool grounded){
		...
		if (absSpeed < speedLimit){
			if (m_speed < 0.0f && demandedForce > 0.0f || m_speed > 0.0f && demandedForce < 0.0f){
                // 不要clamp与速度方向相反的驱动力 Do not clamp the drive force that opposes the speed direction
            }
            else {
			...
			if (m_speed < 0.0f && demandedForce > 0.0f || m_speed > 0.0f && demandedForce < 0.0f){
                // 与速度方向相反的驱动力被加到实际的阻力速度中 Drive force that opposes the speed direction is added to the actual resistive speed
	...
	/// <summary>
    /// 设定轮胎模型 Set the visual transform for the wheel
    /// </summary>
    /// <param name="wd"></param>
	...
	void DisableCollidersRaycast ()
		...
		for (int i=0, c=m_colliders.Length; i<c; i++)
			{
                UnityEngine.GameObject go = m_colliders[i].gameObject;
